...
const MencoProject = ({data, transitionStatus, entry, exit}) => {
    ...
    return(
        ...
        <Wrapper >
            ...
            <img width="100%" src={pascoesGif}/>
            
            <p><b>Solution: </b>{post.intro3}</p>
            <p>{post.intro4}</p>
            <p>view the site prototype: <a href={post.gitlink}> here - invision link</a>  or see the process in detail below</p>
...
export const query = graphql`
  query {
    markdownRemark(frontmatter: {title: {eq: "Pascoes Plumbing" } }
        ){
        html
      frontmatter {
            title
    ...
    images: allFile(
        filter: {
          absolutePath: { regex: "/projects/pascoes-plumbing/" }
    ...
    menco: file(relativePath: { eq: "social-icons/logo.png" }) {
        childImageSharp {
          fluid(maxWidth: 50) {
            ...GatsbyImageSharpFluid
          }
